...
# Load environment variables from Jenkins (with defaults)
PARAM1=${PARAM1:-""}
...
TASK=${TASK:-"misc:run"}
RERUNS=${reruns:-0}
TAGS=${TAGS:-"unclaimed"}   # fixed (no spaces in assignment)
...
# Apply marker selection if TAGS is given
if [ -n "$TAGS" ]; then
    PYTEST_CMD="$PYTEST_CMD -m \"$TAGS\""
fi

# Apply test/path selection if PARAM1 is given
if [ -n "$PARAM1" ]; then
    PYTEST_CMD="$PYTEST_CMD $PARAM1"
fi
